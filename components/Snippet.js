...
export default function Snippet({ className, text, ...rest }) {
  ...
  return (
    <div {...rest} className={`flex items-center justify-between gap-x-3 text-sm py-1.5 pl-4 pr-3 dark:text-white border border-gray-300 dark:border-neutral-700 rounded-md ${className && className}`}>
      <div className="truncate">
        <span className="dark:text-white font-medium mr-2.5">$</span>
        {text}
      </div>
      <button data-testid="snippet-btn" title="Copy Code" onClick={copyText} className="flex items-center dark:bg-neutral-800 dark:hover:bg-neutral-700 bg-gray-100 hover:bg-gray-200 border dark:border-neutral-700 px-0.5 py-0.5 rounded-md transition-all">
        {copy ?
          <>
